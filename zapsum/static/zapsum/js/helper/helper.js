$(document).ready(function(){	
	/*********************************************************************************************** ajax create message */
	$('#createMessageForm').on('click', function(event){
		var	name = $('#id_reciever').val(),
			theme = $('#id_theme').val(),
			text = $('#id_text').val();
		...
		$.ajax({
			...
			beforeSend : function(jqXHR,data){
				var	flag_return = false,
					error_reciever = $('#id_reciever').closest('.cell').find('.errorlist'),
					error_theme = $('#id_theme').closest('.cell').find('.errorlist'),
					error_text = $('#id_text').closest('.cell').find('.errorlist');

				if(!$.trim(name)){
					console.log('name');
					error_reciever.text('Пользователя с таким именем не существует');
					flag_return = true;
				}
				else{
					error_reciever.text('');
				...
				if(!$.trim(theme)){
					console.log('theme');
					error_theme.text('Это обязательное поле');
					flag_return = true;
				}
				else{
					error_theme.text('');
				...
				if(!$.trim(text)){
					console.log('text');
					error_text.text('Это обязательное поле');
					flag_return = true;
				}
				else{
					error_text.text('');
